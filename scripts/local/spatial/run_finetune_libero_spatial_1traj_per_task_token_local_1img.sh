...
"${TORCHRUN_BIN}" --standalone --nnodes 1 --nproc-per-node 1 vla-scripts/finetune.py \
  ...
  --run_id_note "VLA-Adapter--token--1img--${data_name}--${current_time}" \
  > "logs/VLA-Adapter--token--1img--${data_name}--${current_time}.log" 2>&1 &

echo "[INFO] Launched local finetune (tokenized actions, 1 image input) for ${data_name}."
echo "[INFO] Log file: logs/VLA-Adapter--token--1img--${data_name}--${current_time}.log"
