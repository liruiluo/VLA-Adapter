...
# 1-GPU (>=80GB) training VLA-Adapter-Pro on LIBERO-Goal
...
"${TORCHRUN_BIN}" --standalone --nnodes 1 --nproc-per-node 1 vla-scripts/finetune.py \
  ...
  --run_id_note "VLA-Adapter--goal-1GPU--${current_time}" \
  > "logs/VLA-Adapter--goal-1GPU--${current_time}.log" 2>&1

echo "[INFO] Finished 1-GPU finetune job for ${data_name}."
echo "[INFO] Log file: logs/VLA-Adapter--goal-1GPU--${current_time}.log"
