...
# 1-GPU MoE-LoRA training on LIBERO-Goal (offline subset: 1 traj / task).
...
"${TORCHRUN_BIN}" --standalone --nnodes 1 --nproc-per-node 1 vla-scripts/finetune.py \
  ...
  --run_id_note "VLA-Adapter-MoELoRA--goal-1GPU--${data_name}--${current_time}" \
  "$@" \
  > "logs/VLA-Adapter-MoELoRA--goal-1GPU--${data_name}--${current_time}.log" 2>&1
