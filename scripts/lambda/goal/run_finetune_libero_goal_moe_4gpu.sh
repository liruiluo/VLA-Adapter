...
# 4-GPU (>=80GB total) MoE-LoRA training on LIBERO-Goal
...
"${TORCHRUN_BIN}" --standalone --nnodes 1 --nproc-per-node 4 vla-scripts/finetune.py \
  ...
  --run_id_note "VLA-Adapter-MoELoRA--goal-4GPU--${current_time}" \
  "$@" \
  > "logs/VLA-Adapter-MoELoRA--goal-4GPU--${current_time}.log" 2>&1

echo "[INFO] Finished 4-GPU MoE-LoRA finetune job for ${data_name}."
echo "[INFO] Log file: logs/VLA-Adapter-MoELoRA--goal-4GPU--${current_time}.log"
