...
# 4-GPU (>=80GB total) training VLA-Adapter-Pro on LIBERO-Object (offline subset: 1 traj / task).
...
"${TORCHRUN_BIN}" --standalone --nnodes 1 --nproc-per-node 4 vla-scripts/finetune.py \
  ...
  --run_id_note "VLA-Adapter--token--object-4GPU--1img--${data_name}--${current_time}" \
  "$@" \
  > "logs/VLA-Adapter--token--object-4GPU--1img--${data_name}--${current_time}.log" 2>&1

echo "[INFO] Finished 4-GPU finetune job for ${data_name}."
echo "[INFO] Log file: logs/VLA-Adapter--token--object-4GPU--1img--${data_name}--${current_time}.log"
